...
<style>
    ...
    .product-stats { display:grid; grid-template-columns:repeat(3,1fr); gap:16px; margin-bottom:22px; }
    ...
    .products-table { width:100%; min-width:850px; border-collapse:collapse; }
    .products-table th { padding:13px 16px; color:#718187; background:#f7fafb; font-size:10px; font-weight:800; letter-spacing:.08em; text-align:left; text-transform:uppercase; white-space:nowrap; }
    .products-table td { padding:15px 16px; border-top:1px solid #ebf0f1; color:#3e4d53; font-size:13px; vertical-align:middle; }
    .products-table tbody tr:hover { background:#fbfdfd; }
    
    .product-info { display:flex; align-items:center; gap:12px; }
    .product-img { width:44px; height:44px; border-radius:10px; object-fit:cover; border:1px solid var(--line); background:#f7fafb; }
    .product-avatar-fallback { width:44px; height:44px; border-radius:10px; display:grid; place-items:center; background:#e7f7f7; color:var(--sea); border:1px solid var(--line); font-size:18px; }
    .main-data { display:block; color:var(--navy); font-weight:700; }
    .sub-data { display:block; margin-top:2px; color:#89979c; font-size:11px; }

    .category-badge { display:inline-flex; align-items:center; gap:6px; padding:5px 10px; border-radius:999px; font-size:11px; font-weight:700; color:#0b5875; background:#e8f6f8; white-space:nowrap; }
    ...
    .price-tag { color:#087f7f; font-size:15px; font-weight:800; white-space:nowrap; }
    ...
    body.dark-mode .products-header h1, body.dark-mode .stat-card strong, body.dark-mode .history-toolbar h2,
    body.dark-mode .main-data, body.dark-mode .empty-state strong { color:#fff; }
    body.dark-mode .products-table th { color:#b9cbd0; background:#082838; }
    body.dark-mode .products-table td { border-color:#264b5a; color:#d9e5e8; }
    body.dark-mode .products-table tbody tr:hover { background:#0e3a4e; }
    body.dark-mode .action-button, body.dark-mode .search-box input { color:#e7f1f3; border-color:#315565; background:#0d3a4d; }

    @media(max-width:800px) { .product-stats{grid-template-columns:1fr} }
    @media(max-width:650px) { .products-header,.history-toolbar{align-items:stretch;flex-direction:column}.primary-button,.search-box{width:100%}.stat-card{padding:16px}.stat-card strong{font-size:18px} }
</style>
@endpush
...
<div class="products-page">
    <header class="products-header">
        <div>
            ...
            <p>Gestiona los platillos, bebidas y categorías de la carta.</p>
        </div>
    ...
    <section class="product-stats">
        ...
        <article class="stat-card">
            ...
            <small>Agotados / Inactivos</small>
    ...
    <section class="history-card">
        <div class="history-toolbar">
            <h2>Carta registrada <span class="record-count">{{ $totalProductos }}</span></h2>
            <label class="search-box" for="productSearch">
                <i class="fa-solid fa-magnifying-glass"></i>
                <input id="productSearch" type="search" placeholder="Buscar producto o categoría...">
            </label>
        </div>
                        ...
                        <tr class="product-row">
                            <td>
                                <div class="product-info">
                                    @if($producto->imagen)
                                        <img src="{{ Storage::url($producto->imagen) }}" alt="{{ $producto->nombre }}" class="product-img">
                                    @else
                                    ...
                                    @endif
                                    <div>
                                        <span class="main-data">{{ $producto->nombre }}</span>
                                        <span class="sub-data">{{ Str::limit($producto->descripcion ?? 'Sin descripción', 45) }}</span>
                                    </div>
                                </div>
                            </td>
                            <td>
                            ...
                            <td>
                                @if($producto->estado)
                                    <span class="status-badge status-active"><i class="fa-solid fa-circle"></i> Activo</span>
                                @else
                                    <span class="status-badge status-inactive"><i class="fa-solid fa-circle"></i> Inactivo</span>
                                @endif
                            </td>
                            <td>
                                <div class="actions">
                                    <a class="action-button" href="{{ route('productos.show', $producto) }}" title="Ver detalle">
                                        <i class="fa-regular fa-eye"></i>
                                    </a>
                                    <a class="action-button edit-button" href="{{ route('productos.edit', $producto) }}" title="Editar producto">
                                        <i class="fa-solid fa-pen"></i>
                                    </a>
                                    <form method="POST" action="{{ route('productos.destroy', $producto) }}" onsubmit="return confirm('¿Deseas eliminar este producto?')">
                                        @csrf
                                        @method('DELETE')
                                        <button class="action-button delete-button" type="submit" title="Eliminar producto">
                    ...
                    @empty
                        <tr>
                            <td class="empty-state" colspan="5">
                                ...
                                <span>Agrega el primer platillo o bebida para comenzar.</span>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
